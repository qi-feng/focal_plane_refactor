...
def source_from_row(row: pd.Series) -> SourceDetection:
    kwargs = {attr: float(row[col]) for col, attr in _OPTIONAL.items() if col in row.index and pd.notna(row[col])}
...
def find_source_near(df: pd.DataFrame, x: float, y: float, max_radius: float = 50.0) -> SourceDetection:
    ...
    if dist > max_radius:
        raise ValueError(f'No catalog source found within {max_radius:.1f} pixels of ({x:.1f}, {y:.1f}). Nearest source is {dist:.1f} pixels away.')
